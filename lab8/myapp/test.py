...
from lab8.myapp.models.author import Author
from lab8.myapp.models.app import App
from lab8.myapp.models.user import User
from lab8.myapp.models.currency import Currency
from lab8.myapp.models.user_currency import UserCurrency

from lab8.myapp.utils.currencies_api import get_currencies

from lab8.myapp.myapp import MyRequestHandler, app, author, users
...
class TestModels(unittest.TestCase):
    ...
    def test_user_currency_valid(self):
        uc = UserCurrency(user_id=1, currency_id="USD")
...
class TestHTTPServer(unittest.TestCase):
    ...
    @patch("myapp.myapp.get_currencies")
    def test_currencies_page_uses_mocked_data(self, mock_get_currencies):
    ...
    @patch("myapp.myapp.get_currencies")
    def test_user_page_subscriptions(self, mock_get_currencies):
        mock_get_currencies.return_value = {
            "USD": 90.0,
            "EUR": 100.0,
            "CNY": 10.0,
        ...
        first_user = users[0]
        path = f"/user?id={first_user.id}"
        status, body = self.http_get(path)
        self.assertEqual(status, 200)
        self.assertIn(first_user.name, body)
